...
const {dealType} = mockjs.mock({
  'dealType|4': [{
    'id|+1': 1,
    'name|+1': ['买入', '卖出', '转入', '转出'],
  }]
});
const {dealCategory} = mockjs.mock({
  'dealCategory|3': [{
    'id|+1': 1,
    'name|+1': ['消费', '投资', '其他'],
  }]
...
const {transferRecord} = mockjs.mock({
  'transferRecord|5': [{
    'id|+1': 1,
    username: '@cname',
    'no|10000000-99999999': 1,
...
router.get('/transferRecord', async (req, res, next) => {
  res.json(resultOk(transferRecord));
});
router.put('/transferRecord/:id?', async (req, res, next) => {
  res.json(resultOk({}));
});
router.delete('/transferRecord/:id?', async (req, res, next) => {
  const n = transferRecord.findIndex(v => req.params.id === String(v.id));
  transferRecord.splice(n, 1);
  res.json(resultOk({}));
});
