...
import { SubAgentRebate } from './SubAgentRebate.model';
...
interface Props extends Partial<Hoc> {
  edit: { visible: boolean; record: SubAgentRebate };
...
@withLocale
@compose(withApollo)
export default class SubAgentRebateEdit extends React.PureComponent<Props, {}> {
  state = {};
  ...
  render(): React.ReactNode {
    ...
    return (
      <Mutation
        mutation={gql`
          mutation editMutation($body: SubAgentRebateEditInput!, $id: Int!) {
            edit(body: $body, id: $id)
              @rest(
                bodyKey: "body"
                path: "/subAgentRebate/:id"
                method: "put"
                type: "SubAgentRebateEditResult"
              ) {
              state
              message
            }
      ...
      >
        {edit => (
          <EditFormUI
            ...
            onSubmit={(values: SubAgentRebate) => {
              return edit({ variables: { body: values, id: values.id } }).then(
                (v: GqlResult<'edit'>) => {
                  writeFragment(client, 'SubAgentRebate', values);
                  this.props.onDone();
