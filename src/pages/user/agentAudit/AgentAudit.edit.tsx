...
import { AgentAudit } from './AgentAudit.model';
...
interface Props extends Partial<Hoc> {
  edit: { visible: boolean; record: AgentAudit };
...
@withLocale
@compose(withApollo)
export default class AgentAuditEdit extends React.PureComponent<Props, {}> {
  state = {};
  ...
  render(): React.ReactNode {
    ...
    return (
      <Mutation
        mutation={gql`
          mutation editMutation($body: AgentAuditEditInput!, $id: Int!) {
            edit(body: $body, id: $id)
              @rest(
                bodyKey: "body"
                path: "/agentAudit/:id"
                method: "put"
                type: "AgentAuditEditResult"
              ) {
              state
              message
            }
      ...
      >
        {edit => (
          <EditFormUI
            ...
            onSubmit={(values: AgentAudit) => {
              return edit({ variables: { body: values, id: values.id } }).then(
                (v: GqlResult<'edit'>) => {
                  writeFragment(client, 'AgentAudit', values);
                  this.props.onDone();
