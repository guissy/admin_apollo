...
import { MemberLabel } from './MemberLabel.model';
...
interface Props extends Partial<Hoc> {
  edit: { visible: boolean; record: MemberLabel };
...
@withLocale
@compose(withApollo)
export default class MemberLabelEdit extends React.PureComponent<Props, {}> {
  state = {};
  ...
  render(): React.ReactNode {
    ...
    return (
      <Mutation
        mutation={gql`
          mutation editMutation($body: MemberLabelEditInput!, $id: Int!) {
            edit(body: $body, id: $id)
              @rest(
                bodyKey: "body"
                path: "/memberLabel/:id"
                method: "put"
                type: "MemberLabelEditResult"
              ) {
              state
              message
            }
      ...
      >
        {edit => (
          <EditFormUI
            ...
            onSubmit={(values: MemberLabel) => {
              return edit({ variables: { body: values, id: values.id } }).then(
                (v: GqlResult<'edit'>) => {
                  writeFragment(client, 'MemberLabel', values);
                  this.props.onDone();
