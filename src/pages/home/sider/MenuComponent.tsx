...
export default class MenuComponent extends React.PureComponent<MenusProps, MenusState> {
  ...
  render() {
    ...
    const { mode, theme, site = () => '', login, setting = {} as SettingState, logo } = this.props;
    const { openKeys, collapsed } = this.state;
    const menuStyle = setting.nav === 'left' ? { padding: '16px 0' } : {};
    const inlineCollapsed = mode === 'inline' ? { inlineCollapsed: collapsed } : {};
    return (
      <MenuWrap>
        {logo}
        <MenuContainer
