...
interface Props extends Partial<Hoc> {
  edit: { visible: boolean; record: TransferRecord };
...
@withLocale
@compose(withApollo)
export default class TransferRecordEdit extends React.PureComponent<Props, {}> {
  state = {};
  ...
  render(): React.ReactNode {
    ...
    return (
      <Mutation
        mutation={gql`
          mutation editMutation($body: TransferRecordEditInput!, $id: Int!) {
            edit(body: $body, id: $id)
              @rest(
                bodyKey: "body"
                path: "/transferRecord/:id"
                method: "put"
                type: "TransferRecordEditResult"
              ) {
              state
              message
            }
      ...
      >
        {edit => (
          <EditFormUI
            ...
            onSubmit={(values: TransferRecord) => {
              return edit({ variables: { body: values, id: values.id } }).then(
                (v: GqlResult<'edit'>) => {
                  writeFragment(client, 'TransferRecord', values);
                  this.props.onDone();
