...
import { EmailManage } from './EmailManage.model';
...
interface Props extends Partial<Hoc> {
  edit: { visible: boolean; record: EmailManage };
...
@withLocale
@compose(withApollo)
export default class EmailManageEdit extends React.PureComponent<Props, {}> {
  state = {};
  ...
  render(): React.ReactNode {
    ...
    return (
      <Mutation
        mutation={gql`
          mutation editMutation($body: EmailManageEditInput!, $id: Int!) {
            edit(body: $body, id: $id)
              @rest(
                bodyKey: "body"
                path: "/emailManage/:id"
                method: "put"
                type: "EmailManageEditResult"
              ) {
              state
              message
            }
      ...
      >
        {edit => (
          <EditFormUI
            ...
            onSubmit={(values: EmailManage) => {
              return edit({ variables: { body: values, id: values.id } }).then(
                (v: GqlResult<'edit'>) => {
                  writeFragment(client, 'EmailManage', values);
                  this.props.onDone();
