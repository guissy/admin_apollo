...
interface Props extends Partial<Hoc> {
  edit: { visible: boolean; record: IpBlackList };
...
@withLocale
@compose(withApollo)
export default class IpBlackListEdit extends React.PureComponent<Props, {}> {
  state = {};
  ...
  render(): React.ReactNode {
    ...
    return (
      <Mutation
        mutation={gql`
          mutation editMutation($body: IpBlackListEditInput!, $id: Int!) {
            edit(body: $body, id: $id)
              @rest(
                bodyKey: "body"
                path: "/ipBlackList/:id"
                method: "put"
                type: "IpBlackListEditResult"
              ) {
              state
              message
            }
      ...
      >
        {edit => (
          <EditFormUI
            ...
            onSubmit={(values: IpBlackList) => {
              return edit({ variables: { body: values, id: values.id } }).then(
                (v: GqlResult<'edit'>) => {
                  writeFragment(client, 'IpBlackList', values);
                  this.props.onDone();
