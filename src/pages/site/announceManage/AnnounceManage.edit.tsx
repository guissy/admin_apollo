...
import { AnnounceManage } from './AnnounceManage.model';
...
interface Props extends Partial<Hoc> {
  edit: { visible: boolean; record: AnnounceManage };
...
@withLocale
@compose(withApollo)
export default class AnnounceManageEdit extends React.PureComponent<Props, {}> {
  state = {};
  ...
  render(): React.ReactNode {
    ...
    return (
      <Mutation
        mutation={gql`
          mutation editMutation($body: AnnounceManageEditInput!, $id: Int!) {
            edit(body: $body, id: $id)
              @rest(
                bodyKey: "body"
                path: "/announceManage/:id"
                method: "put"
                type: "AnnounceManageEditResult"
              ) {
              state
              message
            }
      ...
      >
        {edit => (
          <EditFormUI
            ...
            onSubmit={(values: AnnounceManage) => {
              return edit({ variables: { body: values, id: values.id } }).then(
                (v: GqlResult<'edit'>) => {
                  writeFragment(client, 'AnnounceManage', values);
                  this.props.onDone();
