...
interface Props extends Partial<Hoc> {
  edit: { visible: boolean; record: ProxyPromotion };
...
@withLocale
@compose(withApollo)
export default class ProxyPromotionEdit extends React.PureComponent<Props, {}> {
  state = {};
  ...
  render(): React.ReactNode {
    ...
    return (
      <Mutation
        mutation={gql`
          mutation editMutation($body: ProxyPromotionEditInput!, $id: Int!) {
            edit(body: $body, id: $id)
              @rest(
                bodyKey: "body"
                path: "/proxyPromotion/:id"
                method: "put"
                type: "ProxyPromotionEditResult"
              ) {
              state
              message
            }
      ...
      >
        {edit => (
          <EditFormUI
            ...
            onSubmit={(values: ProxyPromotion) => {
              return edit({ variables: { body: values, id: values.id } }).then(
                (v: GqlResult<'edit'>) => {
                  writeFragment(client, 'ProxyPromotion', values);
                  this.props.onDone();
