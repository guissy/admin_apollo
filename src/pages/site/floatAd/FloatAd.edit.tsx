...
import { FloatAd } from './FloatAd.model';
...
interface Props extends Partial<Hoc> {
  edit: { visible: boolean; record: FloatAd };
...
@withLocale
@compose(withApollo)
export default class FloatAdEdit extends React.PureComponent<Props, {}> {
  state = {};
  ...
  render(): React.ReactNode {
    ...
    return (
      <Mutation
        mutation={gql`
          mutation editMutation($body: FloatAdEditInput!, $id: Int!) {
            edit(body: $body, id: $id)
              @rest(
                bodyKey: "body"
                path: "/floatAd/:id"
                method: "put"
                type: "FloatAdEditResult"
              ) {
              state
              message
            }
      ...
      >
        {edit => (
          <EditFormUI
            ...
            onSubmit={(values: FloatAd) => {
              return edit({ variables: { body: values, id: values.id } }).then(
                (v: GqlResult<'edit'>) => {
                  writeFragment(client, 'FloatAd', values);
                  this.props.onDone();
