...
interface Props extends Partial<Hoc> {
  edit: { visible: boolean; record: AdHome };
...
@withLocale
@compose(withApollo)
export default class AdHomeEdit extends React.PureComponent<Props, {}> {
  state = {};
  ...
  render(): React.ReactNode {
    ...
    return (
      <Mutation
        mutation={gql`
          mutation editMutation($body: AdHomeEditInput!, $id: Int!) {
            edit(body: $body, id: $id)
              @rest(bodyKey: "body", path: "/adHome/:id", method: "put", type: "AdHomeEditResult") {
              state
              message
            }
          }
      ...
      >
        {edit => (
          <EditFormUI
            ...
            onSubmit={(values: AdHome) => {
              return edit({ variables: { body: values, id: values.id } }).then(
                (v: GqlResult<'edit'>) => {
                  writeFragment(client, 'AdHome', values);
                  this.props.onDone();
