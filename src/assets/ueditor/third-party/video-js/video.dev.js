...
vjs.CoreObject.extend = function(props) {
  ...
  init =
    props['init'] || props.init || this.prototype['init'] || this.prototype.init || function() {};
...
vjs.addClass = function(element, classToAdd) {
  if ((' ' + element.className + ' ').indexOf(' ' + classToAdd + ' ') == -1) {
    element.className =
      element.className === '' ? classToAdd : element.className + ' ' + classToAdd;
  }
};
...
vjs.DurationDisplay.prototype.updateContent = function() {
  ...
  if (duration) {
    this.content.innerHTML =
      '<span class="vjs-control-text">Duration Time </span>' + vjs.formatTime(duration); // label the duration time for screen reader users
  }
};
...
function createMethod(methodName) {
  return function() {
    throw new Error(
      'The "' + methodName + '" method is not available on the playback technology\'s API'
    );
  };
}
...
vjs.TextTrack = vjs.Component.extend({
  ...
  init: function(player, options) {
    ...
    this.id_ =
      options['id'] || 'vjs_' + options['kind'] + '_' + options['language'] + '_' + vjs.guid++;
