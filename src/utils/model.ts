...
import { default as request, ResponseDetail } from './request';
...
export function loadEffect(call: Function, put: Function, namespace: string) {
  return {
    load: function*(asyncFetch: Function, payload: object, resultOpt: ResultOpt = DEFAULT_OPT) {
      const { data, attributes, success, loading } = defaults(DEFAULT_OPT, resultOpt);
      ...
      return showMessageForResult(result, success);
    },
    http: {
      *get(url: string, params: object, resultOpt: ResultOpt = DEFAULT_OPT) {
        const { data, attributes, success, loading } = defaults(DEFAULT_OPT, resultOpt);
        yield put({ type: `${namespace}/@@load`, payload: { [loading]: true } });
        const result: Result<object> = yield request(`${url}?${stringify(params)}`, {
          method: 'GET'
        });
        if (Number(result.state) === 0 && result.data) {
          const other = {
      ...
      *post(url: string, params: object, resultOpt: ResultOpt = DEFAULT_OPT) {
        const { data, attributes, success, loading } = defaults(DEFAULT_OPT, resultOpt);
        yield put({ type: `${namespace}/@@load`, payload: { [loading]: true } });
        const result: Result<object> = yield request(`${url}`, {
          method: 'POST',
      ...
      *put(url: string, params: object, resultOpt: ResultOpt = DEFAULT_OPT) {
        const { data, attributes, success, loading } = defaults(DEFAULT_OPT, resultOpt);
        yield put({ type: `${namespace}/@@load`, payload: { [loading]: true } });
        const result: Result<object> = yield request(`${url}`, {
          method: 'PUT',
